...
impl SetupObj {
    ...
    fn try_to_access_config_data(&mut self) -> std::result::Result<std::fs::File, std::io::Error> {
        let path = env::current_dir().unwrap().to_str().unwrap().to_owned() + "/src/config.json";
